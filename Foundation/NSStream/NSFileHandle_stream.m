...
-(void)synchronizeFile {
   [NSException raise:NSFileHandleOperationException format:@"-[%@ %s]: Operation not supported",[self _KVO_swizzledClass],sel_getName(_cmd)];
}

-(uint64_t)offsetInFile {
   [NSException raise:NSFileHandleOperationException format:@"-[%@ %s]: Illegal seek",[self _KVO_swizzledClass],sel_getName(_cmd)];
   return 0;
}
